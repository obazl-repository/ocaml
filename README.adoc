...
* New name: `ocamlcc` - the OCaml Compiler Collection. Analogous to
  `GCC`, the GNU Compiler Collection. Not only do we have multiple
  compilers - `ocamlc.byte`, `ocamlopt.byte`, etc., the compilers
  themselves may serve to drive a C toolchain, to compile, assemble,
  and link C code.
...
* Recursive builds. Well, at least quasi-recursive. There is only one
  target for building a compiler, `bin:ocamlcc`; to build the
  compiler, this target needs a compiler, which it gets from a
  toolchain, which depends on `bin:ocamlc`, which needs a
  compiler, ... recursively, until we get to the base case, the
  precompiled `boot/ocamlc` compiler. So the boot compiler
  (`boot/ocamlc`) builds the stage 1 compiler which builds the stage 2
  compiler which builds the stage 3 compiler. See
  link:bzl/docs/staged_builds.adoc[Staged builds].
...
* Revised configuration logic. The goal is to eliminate dependency on
  ...
  code is in link:config[//config] and
  link:bzl/docs/notes/autoconf.adoc[notes/autoconf].
...
* Use the link command from the Bazel CC toolchain. OBazl now uses
  ...
  (`Config.mkexe`). For more info see
  link:bzl/docs/ocaml_cc_config.adoc[ocaml_cc_config]. [TODO: same
  thing for the assemble command `Config.asm`]

* Revised link:bzl/docs/preprocessing.adoc[preprocessing]. OBazl eliminates shell scripts and tools,
...
4. Build the Big Four compiler variants:
...
NOTE: Cross-compiling has not yet been adapted to use the new bootstrapping discipline.
