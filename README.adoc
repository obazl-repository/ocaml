...
* Revised configuration logic. The goal is to eliminate dependency on
  ...
  code is in link:config[//config]. For more info see
...
* Use the link command from the Bazel CC toolchain. OBazl now uses
  ...
  link:bzl/docs/ocaml_cc_config.adoc[ocaml_cc_config] and
  link:bzl/docs/notes/linking.adoc[notes/linking]. [TODO: same thing
  for the assemble command `Config.asm`]
