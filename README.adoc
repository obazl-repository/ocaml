...
6. Use a debug variant of the runtime (instead of `build boot/bin:ocamlrund`):

    $ bazel build boot/bin:ocamlrun -c dbg

6. Cross-compile:

  a. mac->linux:  `$ bazel build boot/bin:ocamlrun --config=mac_linuxamd64`
  a. linux->mac:  `$ bazel build boot/bin:ocamlrun --config=linuxamd64_macamd64`
