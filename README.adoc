...
+
5. Use Zig:  `$ bazel run boot/compiler --config=maczig` (or `--config=linuxzig`)

6. Cross-compile:

  a. mac->linux:  `$ bazel run boot/bin:ocamlrun --config=mac_linuxamd64`
  a. linux->mac:  `$ bazel run boot/bin:ocamlrun --config=linuxamd64_macamd64`
