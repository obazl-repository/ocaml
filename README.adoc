...
This target builds all main targets.  See link:BUILD.bazel[BUILD.bazel] for the list.

The compiler targets are in `//boot`, `//boot/bin`, `//boot/lib`, and
`//boot/compiler`. For example, `$ bazel run boot/compiler --
-version`.
