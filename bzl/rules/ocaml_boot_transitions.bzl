...
def _ocaml_in_transition_impl(settings, attr):
    ...
    if protocol == "unspecified": ## building from cmd line
        ...
        fail("Protocol supported for this target: %s" % protocol)
...
def _boot_ocamlc_byte_in_transition_impl(settings, attr):
    debug = True
    if debug: print("boot_ocamlc_byte_in_transition: %s" % attr.name)
...
def _boot_ocamlopt_byte_in_transition_impl(settings, attr):
    debug = True
    if debug:
        print("boot_ocamlopt_byte_in_transition: %s" % attr.name)
...
################################################################
boot_ocamlopt_byte_in_transition = transition(
...
def _boot_ocamlopt_opt_in_transition_impl(settings, attr):
    debug = True
    if debug: print("boot_ocamlopt_opt_in_transition: %s" % attr.name)
...
def _boot_ocamlc_opt_in_transition_impl(settings, attr):
    debug = True
    if debug: print("boot_ocamlc_opt_in_transition: %s" % attr.name)
